    ...
    <div class="flex-1 overflow-hidden">
      <van-tabs v-model:active="active">
        <van-tab :title="$t('contactMenu.myCreate')">
          <virtual-list
            v-if="comptGroupList.createdGroup.length > 0"
            class="my_scrollbar flex-1 overflow-y-auto bg-white"
            :data-key="'groupID'"
            :data-sources="comptGroupList.createdGroup"
        ...
        <van-tab :title="$t('contactMenu.myJoin')">
          <virtual-list
            v-if="comptGroupList.joinedGroup.length > 0"
            class="my_scrollbar flex-1 overflow-y-auto bg-white"
            :data-key="'groupID'"
            :data-sources="comptGroupList.joinedGroup"
            :data-component="GenericListItem"
            :estimate-size="88"
            :extra-props="{
              subKey: 'memberCount',
...
const comptGroupList = computed(() => {
  ...
  contactStore.storeGroupList.map((group) => {
    group.ownerUserID === userStore.storeSelfInfo.userID
